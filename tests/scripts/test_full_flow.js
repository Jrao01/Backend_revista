...
async function test() {
    try {
        ...
        const areaRes = await axios.post(`${API_URL}/areas`, {
            nombre: 'Salud',
            color_institucional: '#ff0000'
        }, {
            headers
        });
        const areaId = areaRes.data.area.id;

        const progRes = await axios.post(`${API_URL}/programas`, {
            nombre: 'Medicina',
            area_id: areaId
        }, {
            headers
        ...
        console.log('\n4. Registrando artículo con archivos y firmas digitales...');
        ...
        form.append('firma_originalidad', 'true');
        form.append('firma_etica', 'true');

        const archivos = [{
                field: 'manuscrito_original',
                file: 'manuscrito.docx'
            },
            {
                field: 'pagina_titulo',
                file: 'pagina_titulo.pdf'
            },
            {
                field: 'ficha_autores',
                file: 'ficha_autores.pdf'
            },
            {
                field: 'material_suplementario',
                file: 'material_suplementario.pdf'
            }
        ];
        ...
        const detailRes = await axios.get(`${API_URL}/articulos/${articleId}`, {
            headers
        });
        console.log('Título:', detailRes.data.titulo_es);
        console.log('Firma Originalidad:', detailRes.data.firma_originalidad);
        console.log('Firma Ética:', detailRes.data.firma_etica);
        console.log('Archivos adjuntos (esperados 4):', detailRes.data.archivos_articulos.length);

        // 6. Consultar Perfil con Artículos
        console.log('\n6. Consultando perfil del usuario...');
        const profileRes = await axios.get(`${API_URL}/usuarios/${userId}`, {
            headers
        });
